...
object OfflineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 核心计算过程
    // 1. 训练隐语义模型  case class Rating(user: Int, product: Int, rating: Double)
    ...
    val userRecs = preRating.filter(_.rating > 0)
      ...
      .groupByKey() //转成用户做key，对用户分组，(user, Iterable[(rating.product, rating.rating)])
      .map {
      case (userId, recs) =>
        //最常用的分组排序 limit
        //sortWith 比较函数 大于0 会把大的放前面  降序

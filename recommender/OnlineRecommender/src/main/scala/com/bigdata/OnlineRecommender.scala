...
object OnlineRecommender {
  ...
  val PRODUCT_RECS = "ProductRecs" //用的隐喻义模型，商品特征向量做的余弦相似度的相似商品
  ...
  def main(args: Array[String]): Unit = {
    ...
    val simProductsMatrix = spark.read
      ...
      .collectAsMap() //ProductRecs(productId: Int, recs: Seq[Recommendation])
    // 定义广播变量
      ...
      rdds =>
        rdds.foreach {
          case (userId, productId, score, timestamp) =>
            ...
            //list键名为uid:USERID，值格式是 PRODUCTID:SCORE
